...
endpoints = {
    "search": "https://api.isthereanydeal.com/games/search/v1",
...
def get_more_info(appid):
    ...
    if resp.status_code == 200:
        ...
        try:
            sysreq_min = data.get("pc_requirements", {}).get(
                "minimum", "Keine Systemanforderungen verfügbar"
            )
            sysreq_rec = data.get("pc_requirements", {}).get(
                "recommended", "Keine Systemanforderungen verfügbar"
            )
...
def display_counter_results():
    ...
    if st.session_state.counter < 1:
        st.session_state.message_container.warning(
            "Keine Deals in den ausgewählten Shops gefunden"
        )

    if st.session_state.counter == 1:
        st.session_state.message_container.success(
            f"{st.session_state.counter} Spiel gefunden!",
            icon=":material/check_circle:",
        )

    if st.session_state.counter > 1:
        st.session_state.message_container.success(
            f"{st.session_state.counter} Spiele gefunden!",
            icon=":material/check_circle:",
        )
...
def display_best_deals(deal_shops):
    ...
    with st.expander(
        expanded=True, label="Beste Preise", icon=":material/euro_symbol:"
    ):
...
def display_game_metadata(info):
    ...
    if release:
        with st.expander("Releasedatum", expanded=False):
            st.badge(
                f"{datetime.strptime(release, '%Y-%m-%d').strftime('%d.%m.%Y')}",
                color="red",
            )
    else:
...
def display_game_card(info, deal_shops):
    ...
    with st.container(border=True, width="stretch", horizontal=False):
        st.subheader(
            f"{st.session_state.counter}. {info.get('title')}",
            divider="red",
        )
...
def get_secrets():
    try:
        ...
        st.error(
            "API Key nicht gefunden. Bitte füge deinen API Key in den Streamlit-Secrets hinzu."
        )
        st.stop()
...
st.title("Hi!")
st.markdown(
    "Gib den **Namen** eines Spiels ein und finde den <ins>günstigsten</ins> Preis.",
    unsafe_allow_html=True,
)
...
with col1:
    title = st.text_input(
        ...
        value=st.session_state.random_game,
        label_visibility="collapsed",
    )
with col2:
    randomizer_button = st.button(
        "", on_click=get_random_game_title, icon=":material/autorenew:", width="stretch"
    )
    ...
    if not title.strip():
        ...
        with st.spinner("Suche nach Spielen..."):
            ...
            with ThreadPoolExecutor(
                max_workers=5
            ) as executor:  # max 5 Worker gleichzeitig
                # Alle IDs sammeln
                ...
                for future in as_completed(future_to_id):
                    ...
                    # Shops filtern
                    deal_shops = filter_deals_by_shops(
                        deals, st.session_state.selected_shops, shops_dict
                    )
